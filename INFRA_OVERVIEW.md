...
- Install packages (`vlan`, `wireguard`, `tailscale`).
- Configure VLAN subinterfaces (e.g., `eth0.10`, `eth0.20`) via `networkd` or `netplan` templates.
- Deploy firewall rules and VPN configuration.

**VLAN Setup**:
- Define VLAN interfaces (`eth0.<VID>`) with IPs (e.g., `192.168.10.1/24`).
- Map subnets: IoT (10), Guest (20), Management (30).
...
- On router VM, advertise VLAN subnets:
  ...
  ```
- Forward UDP port `41641` (Tailscale) and `51820` (WireGuard) from the physical router to the VM’s IP.
